...
@section('content')
    <div class="bg-white p-6 rounded-lg shadow">
        <h1 class="text-2xl font-bold mb-6">{{ __('Record Stock Adjustment') }}</h1>
        ...
        <form method="POST" action="{{ route('inventory.adjustments.store') }}" class="space-y-6">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label for="quantity" class="block text-sm font-medium text-gray-700">{{ __('Quantity (Positive to add, Negative to remove)') }}</label>
                    <input type="number" name="quantity" id="quantity" value="{{ old('quantity') }}" class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500" required>
                    @error('quantity') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label for="reason" class="block text-sm font-medium text-gray-700">{{ __('Reason') }}</label>
                    <input type="text" name="reason" id="reason" value="{{ old('reason') }}" class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500" required>
                    @error('reason') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label for="adjustment_date" class="block text-sm font-medium text-gray-700">{{ __('Adjustment Date') }}</label>
                    <input type="date" name="adjustment_date" id="adjustment_date" value="{{ old('adjustment_date') }}" class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500" required>
