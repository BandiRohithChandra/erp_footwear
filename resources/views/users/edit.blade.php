...
@section('content')
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <form method="POST" action="{{ route('settings.users.update', $user) }}" class="bg-white p-6 rounded-lg shadow space-y-4">
            @csrf
            @method('PUT')
            <div>
                <label for="name" class="block text-gray-700 font-medium mb-2">{{ __('Name') }}</label>
                <input type="text" id="name" name="name" value="{{ old('name', $user->name) }}" class="w-full p-2 border rounded-lg focus:ring-2 focus:ring-blue-500 @error('name') border-red-500 @enderror" required>
                @error('name')
            ...
            <div>
                ...
                <input type="email" id="email" name="email" value="{{ old('email', $user->email) }}" class="w-full p-2 border rounded-lg focus:ring-2 focus:ring-blue-500 @error('email') border-red-500 @enderror" required>
                @error('email')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            <div>
                <label for="password" class="block text-gray-700 font-medium mb-2">{{ __('Password (Leave blank to keep unchanged)') }}</label>
                <input type="password" id="password" name="password" class="w-full p-2 border rounded-lg focus:ring-2 focus:ring-blue-500 @error('password') border-red-500 @enderror">
                @error('password')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select id="manager_id" name="manager_id" class="w-full p-2 border rounded-lg focus:ring-2 focus:ring-blue-500 @error('manager_id') border-red-500 @enderror">
                    ...
                    @foreach (\App\Models\User::where('id', '!=', $user->id)->get() as $manager)
                        <option value="{{ $manager->id }}" {{ old('manager_id', $user->manager_id) == $manager->id ? 'selected' : '' }}>{{ $manager->name }}</option>
                    @endforeach
                </select>
                @error('manager_id')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div>
                <label class="inline-flex items-center">
                    <input type="checkbox" name="is_remote" value="1" {{ old('is_remote', $user->is_remote) ? 'checked' : '' }} class="rounded border-gray-300 text-blue-500 focus:ring-blue-500">
            ...
            <div>
                ...
                @foreach ($roles as $role)
                    <div class="flex items-center">
                        <input type="checkbox" id="role_{{ $role->name }}" name="roles[]" value="{{ $role->name }}" {{ $user->hasRole($role->name) ? 'checked' : '' }} class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
