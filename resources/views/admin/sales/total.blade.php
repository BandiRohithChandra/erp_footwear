...
<div class="container mx-auto px-4 py-8" x-data="{ openModal: false, selectedOrder: null }">
    ...
    <div class="flex flex-col md:flex-row md:justify-between md:items-center mb-8">
        <h1 class="text-3xl font-bold text-gray-800">Total Sales Dashboard</h1>
        <a href="{{ route('admin.online') }}" class="mt-4 md:mt-0 inline-flex items-center px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg shadow hover:bg-blue-700 transition">
            Back to Dashboard
        </a>
    </div>

    <!-- Summary Cards -->
    <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        <a href="{{ route('admin.sales.details') }}" class="block bg-white shadow-lg rounded-xl p-6 flex flex-col justify-between border-t-4 border-green-500 hover:shadow-2xl transition">
            <div class="flex items-center justify-between mb-4">
    <h2 class="text-xl font-semibold text-gray-700">Total Sales Amount</h2>
    <span class="text-green-500 text-2xl font-bold">₹</span>
        ...
        <a href="{{ route('admin.orders.completed') }}" class="block bg-white shadow-lg rounded-xl p-6 flex flex-col justify-between border-t-4 border-blue-500 hover:shadow-2xl transition">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-xl font-semibold text-gray-700">Completed Orders</h2>
                <i class="fas fa-check-circle text-blue-500 text-2xl"></i>
            </div>
            <p class="text-3xl font-bold text-gray-900">{{ $completedOrdersCount }}</p>
        </a>

        <a href="{{ route('admin.orders.pending_payments') }}" class="block bg-white shadow-lg rounded-xl p-6 flex flex-col justify-between border-t-4 border-red-500 hover:shadow-2xl transition">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-xl font-semibold text-gray-700">Pending Payments</h2>
                <i class="fas fa-clock text-red-500 text-2xl"></i>
            </div>
            <p class="text-3xl font-bold text-gray-900">
                ₹{{ number_format(collect($orders)->sum(fn($o) => $o['total'] - $o['paid_amount']), 2) }}
            </p>
        </a>
    </div>

    <!-- Due Payments -->
    <h2 class="text-2xl font-semibold text-gray-800 mb-4">Due Payments</h2>

    <!-- Desktop Table -->
    <div class="hidden md:block bg-white shadow-lg rounded-xl p-6 overflow-x-auto mb-6">
            ...
            <tbody>
                @forelse($orders as $order)
                    <tr class="border-b hover:bg-gray-50 cursor-pointer"
                        ...
                        <td class="px-4 py-2 text-red-500 font-bold">₹{{ number_format($order['total'] - $order['paid_amount'], 2) }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="text-center py-4 text-gray-500">No due payments found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="md:hidden space-y-4">
        @forelse($orders as $order)
            <div class="bg-white shadow rounded-xl p-4 cursor-pointer"
                ...
                <div><span class="text-gray-600">Sales Person: </span>{{ $order['user']['name'] }}</div>
        ...
        @empty
            <div class="text-center text-gray-500">No due payments found.</div>
        @endforelse
    </div>

    <!-- Professional Modal -->
    <div x-show="openModal" x-transition class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 px-4" x-cloak>
        <div class="bg-white rounded-2xl shadow-2xl w-full sm:w-11/12 md:w-3/4 lg:w-2/3 max-w-2xl max-h-[85vh] overflow-y-auto p-6 relative">
